...
// ============================================================
// This Phaser 3 game was split from one game.js into the src/*.js files
// below. They are CLASSIC <script>s (no ES modules), so all top-level
// symbols share one global scope — exactly as the old single file did.
// Use the anchors below (function names, CFG keys, class names, log tags)
// as grep targets instead of line numbers, which drift with every edit.
//
//   grep -rn "functionName" src/        # find a definition
//   grep -rn "CFG\.KEY_NAME" src/       # find all uses of a config key
//   grep -rn "_log('\\[BUILD" src/      # find all build log entries
//
// ── FILE STRUCTURE (load order; see index.html) ──────────────
//   src/version.js        VERSION + _fmtVersion  (← VERSION lives HERE now)
//   src/config.js         THIS FILE: CFG, _isMobile, ENEMY_STATS, ENEMY_LOOT,
//                         CHARS, STATE, biome/RNG/log helpers, BIOME_COLORS
//   src/audio.js          Music, SFX
//   src/sprites.js        all draw*() textures, getControls, makeScaleProxy,
//                         buildTextures
//   src/scenes-menu.js    DEFAULT_BINDINGS + Boot/Controls/ModeSelect/
//                         Settings/CharSelect scenes
//   src/scene-game.js     GameScene (the bulk of the game; RECIPES is a static)
//   src/scene-gameover.js GameOverScene (death + stats + leaderboard)
